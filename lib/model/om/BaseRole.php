...
abstract class BaseRole extends BaseObject  implements Persistent {
  ...
  const PEER = 'RolePeer';
	 ...
	 * related UserHasRoles from storage. If this Role is new, it will return
		...
		if ($this->collUserHasRoles === null) {
			if ($this->isNew()) {
			   $this->collUserHasRoles = array();
				...
				$criteria->add(UserHasRolePeer::ROLE_ID, $this->id);
	 ...
	 */
	public function getUserHasRolesJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(RolePeer::DATABASE_NAME);
		}
		...
		if ($this->collUserHasRoles === null) {
			if ($this->isNew()) {
				...
				$this->collUserHasRoles = UserHasRolePeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
